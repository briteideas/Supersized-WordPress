...
function sandbox_body_class( $print = true ) {
	...
	if ( is_single() ) {
		...
		// Adds custom post type to body class
		if( isset( $wp_query->post->post_type ) && $wp_query->post->post_type != 'post' )
			$c[] = $wp_query->post->post_type;
...
function kadg_get_projects_list(){
		$args = array( 'post_type' => 'project', 'num_posts' => -1 );
...
function kadg_render_json_attachments(){
...
?>	
    <?php if( $total_attachments ) { ?>	
		...
		<?php for( $i=0; $i<$total_attachments; $i++ ) { ?>
			{image : "<?php echo $attachments[$i]['location']; ?>", title : "<?php echo $attachments[$i]['title']; ?>"} ,
		<?php } ?>
			]
	<?php } ?>
	<?php }
...
function kadg_render_attachments( $attachments ){ 
	$attachments = attachments_get_attachments();
	$total_attachments = count( $attachments );
	...
	<ul>
    <?php for( $i=0; $i<$total_attachments; $i++ ) : ?>
      <?php if( $attachments[$i]['mime'] == 'image/jpeg' ) { ?>
			<?php $size = getimagesize( $attachments[$i]['location'] ); ?>
			<li><img 
				id="image-<?php echo $attachments[$i]['id']; ?>" 
				src="<?php echo $attachments[$i]['location']; ?>"
				alt="<?php echo $attachments[$i]['title']; ?>"
				width="<?php echo $size['width']; ?>"
				height="<?php echo $size['height']; ?>"
				/>
			</li>
			<?php error_log( var_dump(  ) ); ?>
		<?php } ?>
    <?php endfor; ?>
    </ul>
    <?php endif; ?>
